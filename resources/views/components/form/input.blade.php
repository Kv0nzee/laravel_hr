...
<x-form.inputWrapper>
    <input
        ...
        type="{{ $type }}"
        id="{{ $name }}"
        class="block w-full px-6 pt-6 pb-1 text-white rounded-md appearance-none text-md bg-gray-800 focus:outline-none focus:ring-0 peer @error($name) border-red-500 @enderror"
        placeholder=" "
    />
    <label
        for="{{ $name }}"
